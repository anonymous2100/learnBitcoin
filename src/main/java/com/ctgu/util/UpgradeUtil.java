...
import com.ctgu.App;
import com.ctgu.bean.VersionSummary;
import com.ctgu.constant.Constants;
import com.ctgu.ui.dialog.UpdateInfoDialog;
...
public class UpgradeUtil
{
	public static void checkUpdate(boolean initCheck)
	{
		...
		String versionSummaryJsonContent = HttpUtil.get(Constants.CHECK_VERSION_URL);
		if (StringUtils.isEmpty(versionSummaryJsonContent) && !initCheck)
		{
			JOptionPane.showMessageDialog(App.mainFrame, "Check for timeouts, follow GitHub Release!", "Network error",
		...
		else
		{
			if (!initCheck)
			{
				JOptionPane.showMessageDialog(App.mainFrame, "It's the latest version!", "Congratulations",
						JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	/**
	 * Smooth upgrade The version update scripts and sql methods involved are as idempotent as possible to avoid
	 * repeated upgrade operations due to unusual interruptions such as power failures and deaths during the upgrade
	 * process
	 */
	public static void smoothUpgrade()
	{
		...
		else
		{
			...
			String versionSummaryJsonContent = FileUtil.readString(Constants.class.getResource("/version_summary.json"),
					CharsetUtil.UTF_8);
			versionSummaryJsonContent = versionSummaryJsonContent.replace("\n", "");
			VersionSummary versionSummary = JSON.parseObject(versionSummaryJsonContent, VersionSummary.class);
			...
			// If the upgrade is complete and successful, the version number prior to the upgrade is assigned to the
			// current version
