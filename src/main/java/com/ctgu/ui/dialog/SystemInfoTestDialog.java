...
import com.ctgu.util.ConsoleUtil;
...
{
	private static final long serialVersionUID = 3385501384990830070L;
	...
	private void printServices(OperatingSystem os)
	{
		...
		for (OSService s : os.getServices())
		{
			...
			{
				ConsoleUtil.consoleOnly(textArea1,
						String.format(" %5d  %7s  %s", s.getProcessID(), s.getState(), s.getName()));
			}
		}
		i = 0;
		for (OSService s : os.getServices())
	...
	private void printFileSystem(FileSystem fileSystem)
	{
		...
		ConsoleUtil.consoleOnly(textArea1, String.format(" File Descriptors: %d/%d",
